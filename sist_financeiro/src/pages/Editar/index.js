...
export const Editar = (props) => {
    ...
    const[valorLancamentoTarget, setValorLancTarget] = useState('');
    ...
    const converterReal = async (valorLancamentoInput) => {
        var valorLancConvert = valorLancamentoInput.toString().replace(/\D/g, "");
        ...
        var valorSalvar = await valorLancConvert.replace(".", "");
    ...
    useEffect(() => {
        const getLancamento = async () => {
            ...
            .then((response) => {
                ...
                setValorLancTarget(new Intl.NumberFormat('pt-BR', { minimumFractionDigits: 2, currency: 'BRL' }).format(response.data.lancamento.valor));
    ...
    return(
        <Container>
            ...
            <Form onSubmit={editLancamento}>
                ...
                <Label>Valor: </Label>
                <Input type="text" name="valorLancamentoTarget" placeholder="Valor do Lançamento" value={valorLancamentoTarget} onChange={e => converterReal(e.target.value)} />
